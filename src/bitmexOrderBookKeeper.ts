...
export class BitmexOrderBookKeeper extends BaseKeeper {
  ...
  getOrderBookWs(pair: string, depth?: number): OrderBookSchema | null {
    ...
    if (depth === 1) {
      ...
      const bidsUnsorted: OrderBookItem[] = [maxBid];
      const asksUnsorted: OrderBookItem[] = [minAsk];
      return {
        pair,
      ...
      const bidsUnsorted: OrderBookItem[] = _.map(bidsUnsortedRaw, d => ({
        r: d.price,
        a: d.size,
      }));
      const asksUnsorted: OrderBookItem[] = _.map(askUnsortedRaw, d => ({
        r: d.price,
        a: d.size,
      }));
